...
-- это парадигма программирования, что порграмма представлена ввиде множества объектов
-- объекты являются экземплярами опреднного класса
...
-- в объекте должна присутствовать инкапсуляция
...
- `Стек` LIFO  — это область оперативной памяти.
 ...
 ```
- `Куча` — это область оперативной памяти, в которой хранятся данные, созданные во время выполнения программы. Куча используется для динамического выделения памяти для объектов, которые могут изменять размер во время выполнения программы.
 ...
 -- используется для выделения памяти обхектам которые могут изменять размер во время выполнения программы. 
...
1.  `createState()` вызывается 1 раз далее создаётся объект State , затем State привязывается к экземпляру StatefulWidget. Это чтобы State знал с каким виджетом он работает.
2.  `mounted is true` - далее свойство  ***mounted*** в объекте State устанавливается в true. (означает что объект включен в дерево виджетов)
